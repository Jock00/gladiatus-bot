...
class Auction(Settings):
    auction_url = "https://s69-en.gladiatus.gameforge.com/game/index.php"
    data = {
    'doll': '1',
    'qry': '',
    #'itemLevel': '63',
        ...
        if time.lower() == self.time:
            ...
            for elem in elements:
                ...
                try:
                    ...
                    team_mate = elem.xpath(".//*[@class='auction_bid_div']//div[1]//@href")
                    if team_mate:
                        continue
                    
                    auction_id = elem.xpath('.//*[@name="auctionid"]//@value')[0]
                    item_type = elem.xpath('.//*[@name="itemType"]//@value')[0]
                    item_level = elem.xpath('.//*[@name="itemLevel"]//@value')[0]
                    item_price = elem.xpath('.//*[@name="bid_amount"]//@value')[0]
                    ...
                    txt.append(f"Bid for '{name[0]}' ({name[1].split(':')[-1].strip()}) - price: {item_price}" + "\n")
            return txt
        else:
            return [f"You have to wait. Current time is {time.lower()}. Try again when is **{self.time}**"]
